  ...
  return (
    <div className="flex flex-col items-center max-w-5xl mx-auto mt-12 animate-in fade-in duration-500">
      ...
      <div className="w-full relative mb-12 flex shadow-sm rounded-2xl bg-white p-2 border border-slate-200">
        ...
        <button
          onClick={onSearch}
          className="bg-brand-green hover:bg-emerald-600 text-white px-10 rounded-xl font-medium text-lg transition-colors"
        >
          Найти
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full">
        ...
        <div className="flex flex-col">
          ...
          <div className="flex-1 border-2 border-dashed border-brand-blue/30 rounded-3xl bg-white flex flex-col items-center justify-center p-8 text-center hover:bg-slate-50 transition-colors cursor-pointer min-h-[300px]">
            ...
            <p className="text-xl font-medium text-slate-900 mb-2">
              Перетащите файл сюда или <span className="text-brand-blue underline decoration-brand-blue/30 underline-offset-4">выберите вручную</span>
            </p>
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200">
            ...
            <div>
              ...
              <div className="flex border border-slate-200 rounded-lg h-10 overflow-hidden focus-within:border-brand-blue">
                ...
                <div className="flex flex-col border-l border-slate-200 w-8 bg-slate-50">
                  <button onClick={() => setDosage(d => d + 10)} className="flex-1 flex items-center justify-center hover:bg-slate-100 border-b border-slate-200"><ChevronUp size={12} className="text-slate-500"/></button>
                  <button onClick={() => setDosage(d => Math.max(0, d - 10))} className="flex-1 flex items-center justify-center hover:bg-slate-100"><ChevronDown size={12} className="text-slate-500"/></button>
                </div>
              </div>
            </div>
            <div>
          ...
          <div className="mb-8">
            ...
            <input 
              type="range" min="1" max="5" step="1" defaultValue="3"
          ...
          <div className="mb-8">
            <label className="block text-sm font-medium text-brand-blue mb-3">Дополнительные параметры</label>
            <div className="flex flex-col gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-brand-blue focus:ring-brand-blue accent-brand-blue" />
                <span className="text-sm text-slate-600">Только референтные препараты</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-brand-blue focus:ring-brand-blue accent-brand-blue" />
                <span className="text-sm text-slate-600">Только открытый доступ</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" defaultChecked className="w-4 h-4 rounded border-slate-300 text-brand-blue focus:ring-brand-blue accent-brand-blue" />
                <span className="text-sm text-slate-600">Исследования на людях</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-brand-blue focus:ring-brand-blue accent-brand-blue" />
                <span className="text-sm text-slate-600">Полные тексты</span>
              </label>
            </div>
          </div>
